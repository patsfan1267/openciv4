...
//	}
	...
	// Final fallback: never let a city sit idle.
	// Try any building first, then any unit regardless of cost/strategy.
	if (AI_chooseBuilding())
	{
		return;
	}
	...
	// Try each military unit AI type individually
	if (AI_chooseUnit(UNITAI_ATTACK))
	{
		return;
	}
	...
	// Absolute last resort: pick the first trainable unit of any kind
	{
		UnitTypes eFallback = NO_UNIT;
		int iBestCombat = 0;
		for (int iI = 0; iI < GC.getNumUnitClassInfos(); iI++)
		{
			...
			{
				int iCombat = GC.getUnitInfo(eLoopUnit).getCombat();
				if (iCombat > iBestCombat)
				{
					iBestCombat = iCombat;
					eFallback = eLoopUnit;
				}
		...
		if (eFallback != NO_UNIT)
		{
			pushOrder(ORDER_TRAIN, eFallback, UNITAI_ATTACK, false, false, false);
			return;
		}
	}
}
