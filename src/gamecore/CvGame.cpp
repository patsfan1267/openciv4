...
bool CvGame::isBuildingClassMaxedOut(BuildingClassTypes eIndex, int iExtra)
{
	...
	// --- DEBUG TRAP START ---
	// This code catches the crash BEFORE it happens and prints the specific building name to the output window.
	if (getBuildingClassCreatedCount(eIndex) > GC.getBuildingClassInfo(eIndex).getMaxGlobalInstances())
	{
		char szOut[1024];
		sprintf(szOut, "!!! CRITICAL FAILURE !!! eIndex: %d\n", eIndex);
		OutputDebugString(szOut);

		// Safety check to ensure we don't crash while trying to print the name
		if (eIndex >= 0 && eIndex < GC.getNumBuildingClassInfos())
		{
			sprintf(szOut, "!!! BUILDING NAME: %s\n", GC.getBuildingClassInfo(eIndex).getDescription());
			OutputDebugString(szOut);
		}
		else
		{
			OutputDebugString("!!! INDEX IS OUT OF BOUNDS (MEMORY CORRUPTION)\n");
		}
	}
	// --- DEBUG TRAP END ---
