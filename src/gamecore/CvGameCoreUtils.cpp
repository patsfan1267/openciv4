...
int pathDestValid(int iToX, int iToY, const void* pointer, FAStar* finder)
{
	...
	// K-Mod: handle both FAStar and KmodPathFinder calls
	pSelectionGroup = finder ? (CvSelectionGroup*)pointer : ((CvPathSettings*)pointer)->pGroup;
			...
			if ((iMovesLeft < iWorstMovesLeft) || (iMax <= iWorstMax))
			{
				...
				if (pLoopUnit->canFight())
				{
					...
					if (pSelectionGroup->AI_isControlled())
					{
						...
						{
							if (finder && gDLL->getFAStarIFace()->IsPathDest(finder, pToPlot->getX_INLINE(), pToPlot->getY_INLINE()))
							{
...
// K-Mod helper functions for KmodPathFinder
int pathValid_join(FAStarNode* parent, FAStarNode* node, CvSelectionGroup* pSelectionGroup, int iFlags)
{
	...
	if (pSelectionGroup->getDomainType() == DOMAIN_SEA)
	{
		if (pFromPlot->isWater() && pToPlot->isWater())
		{
			if (!(GC.getMapINLINE().plotINLINE(pFromPlot->getX_INLINE(), pToPlot->getY_INLINE())->isWater()) && 
				!(GC.getMapINLINE().plotINLINE(pToPlot->getX_INLINE(), pFromPlot->getY_INLINE())->isWater()))
			{
				return FALSE;
			}
		}
...
int pathAdd(FAStarNode* parent, FAStarNode* node, int data, const void* pointer, FAStar* finder)
{
	...
	CvSelectionGroup* // K-Mod: handle both FAStar and KmodPathFinder calls
	pSelectionGroup = finder ? (CvSelectionGroup*)pointer : ((CvPathSettings*)pointer)->pGroup;
	int iFlags = finder ? gDLL->getFAStarIFace()->GetInfo(finder) : ((CvPathSettings*)pointer)->iFlags;
	// K-Mod end
...
{
	// Internal unit is now half-days; divide by 730 to get year
	return (getTurnMonthForGame(iGameTurn, iStartYear, eCalendar, eSpeed) / 730);
}


// NOTE: Despite the name, this function now returns total HALF-DAYS from epoch
// (reinterpreted so iMonthIncrement=1 means 1 half-day per turn: Morning/Evening)
int getTurnMonthForGame(int iGameTurn, int iStartYear, CalendarTypes eCalendar, GameSpeedTypes eSpeed)
{
	...
	case CALENDAR_DEFAULT:
		...
		for (iI = 0; iI < GC.getGameSpeedInfo(eSpeed).getNumTurnIncrements(); iI++)
		{
			...
			{
				iTurnHalfDay += (GC.getGameSpeedInfo(eSpeed).getGameTurnInfo(iI).iMonthIncrement * GC.getGameSpeedInfo(eSpeed).getGameTurnInfo(iI).iNumGameTurnsPerIncrement);
				iTurnCount += GC.getGameSpeedInfo(eSpeed).getGameTurnInfo(iI).iNumGameTurnsPerIncrement;
			}
			else
			{
				iTurnHalfDay += (GC.getGameSpeedInfo(eSpeed).getGameTurnInfo(iI).iMonthIncrement * (iGameTurn - iTurnCount));
		...
		{
			iTurnHalfDay += (GC.getGameSpeedInfo(eSpeed).getGameTurnInfo(GC.getGameSpeedInfo(eSpeed).getNumTurnIncrements() - 1).iMonthIncrement * (iGameTurn - iTurnCount));
		}
		break;

	case CALENDAR_BI_YEARLY:
		iTurnHalfDay += (2 * iGameTurn * 730);
		break;

	case CALENDAR_YEARS:
	case CALENDAR_TURNS:
		iTurnHalfDay += iGameTurn * 730;
		break;

	case CALENDAR_SEASONS:
		iTurnHalfDay += (iGameTurn * 730) / GC.getNumSeasonInfos();
		break;

	case CALENDAR_MONTHS:
		iTurnHalfDay += (iGameTurn * 730) / GC.getNumMonthInfos();
		break;

	case CALENDAR_WEEKS:
		iTurnHalfDay += iGameTurn * 14;
		break;
