...
void CvGameTextMgr::setDateStr(CvWString& szString, int iGameTurn, bool bSave, CalendarTypes eCalendar, int iStartYear, GameSpeedTypes eSpeed)
{
	...
	case CALENDAR_DEFAULT:
		{
			// getTurnMonthForGame now returns total half-days from epoch
			int iTotalHalfDays = getTurnMonthForGame(iGameTurn, iStartYear, eCalendar, eSpeed);
			int iHalfDayIncrement = getTurnMonthForGame(iGameTurn + 1, iStartYear, eCalendar, eSpeed) - iTotalHalfDays;

			if (iHalfDayIncrement >= 730)
			{
				// Yearly or larger turns: just show year
				szString = szYearBuffer;
			}
			else
			{
				// Convert half-days to days and morning/evening flag
				int iTotalDays = iTotalHalfDays / 2;
				bool bEvening = (iTotalHalfDays % 2) != 0;

				// Convert total days to year + day-of-year
				int iDayOfYear = iTotalDays % 365;
				if (iDayOfYear < 0) { iDayOfYear += 365; }
				...
				for (int m = 0; m < 12; m++)
				{
					if (iDay < daysInMonth[m]) break;
					iDay -= daysInMonth[m];
					iMonth++;
				}
				if (iMonth > 11) iMonth = 11;
				iDay++; // 1-indexed

				if (iHalfDayIncrement >= 56)
				{
					// Monthly turns: show "Month, Year"
					if (bSave)
					{
				...
				{
					// Daily or half-daily turns: show "Day Month, Year" with optional Morning/Evening
					CvWString szTimeOfDay;
					if (iHalfDayIncrement <= 1)
					{
						// Half-day turns: show Morning/Evening
						szTimeOfDay = bEvening ? L" (Evening)" : L" (Morning)";
					}

					CvWString szDay;
					szDay.Format(L"%d ", iDay);
					if (bSave)
					{
						CvWString szDaySave;
						szDaySave.Format(L"-%02d", iDay);
						szString = (szYearBuffer + "-" + GC.getMonthInfo((MonthTypes)iMonth).getDescription() + szDaySave + szTimeOfDay);
					}
					else
					{
						szString = (szDay + GC.getMonthInfo((MonthTypes)iMonth).getDescription() + CvString(", ") + szYearBuffer + szTimeOfDay);
					}
				}
			}
		}
