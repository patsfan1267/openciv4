...
void CvPlayer::acquireCity(CvCity* pOldCity, bool bConquest, bool bTrade, bool bUpdatePlotGroups)
{
	...
	// Give conqueror the old owner's city culture so the culture level
	// (border size) is preserved. Old owner's culture is kept intact
	// so they retain cultural presence in the city.
	if (bConquest)
	{
		aiCulture[getID()] += aiCulture[eOldOwner];
	}
	...
	// Give conqueror +1 culture on tiles in captured city's culture range
	// Old owner's culture is left intact — if they have nearby cities still
	// generating culture, they'll continue to exert border pressure
	if (bConquest)
	{
		for (iDX = -(iRange); iDX <= iRange; iDX++)
		{
			for (iDY = -(iRange); iDY <= iRange; iDY++)
			{
				if (pNewCity->cultureDistance(iDX, iDY) <= iRange)
				{
					pLoopPlot = plotXY(pCityPlot->getX_INLINE(), pCityPlot->getY_INLINE(), iDX, iDY);

					if (pLoopPlot != NULL)
					{
						pLoopPlot->changeCulture(getID(), 1, false);
					}
				}
			}
		}
...
void CvPlayer::setEndTurn(bool bNewValue)
{
	if (isEndTurn() != bNewValue)
	{
		...
		// Autosave right when the human player clicks End Turn,
		// before auto-moves or AI processing begins.
		if (bNewValue && isHuman() && getID() == GC.getGameINLINE().getActivePlayer())
		{
