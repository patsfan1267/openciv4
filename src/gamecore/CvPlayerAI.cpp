...
// K-Mod: Bool version of AI_getPlotDanger that uses the border danger cache
// for early termination. Returns true if any danger is detected.
bool CvPlayerAI::AI_getAnyPlotDanger(CvPlot* pPlot, int iRange, bool bTestMoves, bool bCheckBorder) const
{
	...
	if (bCheckBorder)
	{
		if (iRange >= BORDER_DANGER_RANGE && pPlot->getBorderDangerCache(eTeam))
		{
			return true;
		}
	}
