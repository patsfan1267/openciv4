...
// K-Mod: Static pathfinder instance for optimized path calculations
KmodPathFinder CvSelectionGroup::path_finder;
	...
	if ((getNumUnits() > 0) && (headMissionQueueNode() != NULL))
	{
		...
		if (bDone)
		{
			if (!isBusy())
			{
				...
				// Check if next queued mission is fortify/sleep.
				// Apply directly since these are instant state changes
				// that should work even with 0 movement points remaining.
				CLLNode<MissionData>* pNext = nextMissionQueueNode(headMissionQueueNode());
				if (pNext != NULL &&
					(pNext->m_data.eMissionType == MISSION_FORTIFY ||
					 pNext->m_data.eMissionType == MISSION_SLEEP))
				{
					setActivityType(ACTIVITY_SLEEP);
					clearMissionQueue();
				}
...
{
	// K-Mod: Return end node from optimized pathfinder
...
{
	// K-Mod: Use optimized pathfinder
...
bool CvSelectionGroup::generatePath( const CvPlot* pFromPlot, const CvPlot* pToPlot, int iFlags, bool bReuse, int* piPathTurns) const
{
	...
	path_finder.SetSettings(this, iFlags);
	bool bSuccess = path_finder.GeneratePath(pFromPlot->getX_INLINE(), pFromPlot->getY_INLINE(),
...
{
	// K-Mod: The optimized pathfinder handles its own resets
	// Keep the old reset for any code that might still use it
