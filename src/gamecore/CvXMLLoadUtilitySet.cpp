...
void CvXMLLoadUtility::SetGlobalClassInfo(std::vector<T*>& aInfos, const char* szTagName, bool bTwoPass)
{
	...
	if (gDLL->getXMLIFace()->LocateNode(m_pFXml, szTagName))
	{
		...
		do
		{
			// DEBUG: trace cursor position before read
			{
				TCHAR szDbgTag[256] = {};
				gDLL->getXMLIFace()->GetLastLocatedNodeTagName(m_pFXml, szDbgTag);
				if (iLoopCount < 120)
					fprintf(stderr, "[SGCI %s] iter %d BEFORE read: cursor='%s'\n", szTagName, iLoopCount, szDbgTag);
			}
			...
			// DEBUG: trace cursor position after read
			{
				TCHAR szDbgTag2[256] = {};
				gDLL->getXMLIFace()->GetLastLocatedNodeTagName(m_pFXml, szDbgTag2);
				if (iLoopCount < 120)
					fprintf(stderr, "[SGCI %s] iter %d AFTER  read: cursor='%s' type='%s'\n",
						szTagName, iLoopCount, szDbgTag2,
						pClassInfo->getType() ? pClassInfo->getType() : "(null)");
			}
			...
			if (++iLoopCount > 5000)
			{
				fprintf(stderr, "[SGCI %s] SAFETY LIMIT: aborting after %d iterations\n", szTagName, iLoopCount);
				break;
			}

		} while (gDLL->getXMLIFace()->NextSibling(m_pFXml));
		fprintf(stderr, "[SGCI %s] finished: %d items loaded\n", szTagName, (int)aInfos.size());

		if (bTwoPass)
		{
			fprintf(stderr, "[SGCI %s] starting pass 2...\n", szTagName);
			// if we successfully locate the szTagName node
			if (gDLL->getXMLIFace()->LocateNode(m_pFXml, szTagName))
			{
				gDLL->getXMLIFace()->SetToParent(m_pFXml);
				...
				{
					if (iPass2Count < 5 || iPass2Count % 20 == 0)
					{
						TCHAR szDbgTag[256] = {};
						gDLL->getXMLIFace()->GetLastLocatedNodeTagName(m_pFXml, szDbgTag);
						fprintf(stderr, "[SGCI %s] pass2 iter %d: cursor='%s'\n", szTagName, iPass2Count, szDbgTag);
					}

					SkipToNextVal();	// skip to the next non-comment node

					if (iPass2Count < 5) fprintf(stderr, "[SGCI %s] pass2 iter %d: calling readPass2...\n", szTagName, iPass2Count);
					(*it)->readPass2(this);
					if (iPass2Count < 5) fprintf(stderr, "[SGCI %s] pass2 iter %d: readPass2 returned\n", szTagName, iPass2Count);

					++iPass2Count;

					if (!gDLL->getXMLIFace()->NextSibling(m_pFXml))
					{
						fprintf(stderr, "[SGCI %s] pass2 NextSibling returned false at iter %d\n", szTagName, iPass2Count);
						break;
					}
				}
				fprintf(stderr, "[SGCI %s] pass2 done: %d items processed\n", szTagName, iPass2Count);
			}
			else
			{
				fprintf(stderr, "[SGCI %s] pass2: LocateNode failed\n", szTagName);
			}
		}
	}
}
