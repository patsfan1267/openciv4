...
#ifdef OPENCIV4
	// 64-bit compatibility: size_t is unsigned __int64 on x64
	// Route to unsigned int since serialized values fit in 32 bits
	void Write(size_t value) { Write(static_cast<unsigned int>(value)); }
	void Read(size_t* value) { unsigned int v; Read(&v); *value = v; }
#endif
};
